...
- [ ] Monthly-stats-by-email (test case 5): agent schedules fine via cronjob tool but delivery is `local` only — no email channel in Hermes. Wire `hermes send`/gateway delivery or an email skill if the demo needs real email.
