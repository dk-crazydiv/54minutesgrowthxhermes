...
if [ -n "$PROJECT_ROOT" ] && [ -f "$PROJECT_ROOT/.env" ]; then
    # Symlink, not copy: the repo-local .env (gitignored) is the single source
    # of truth, so a key added there is live for hermes without re-running setup.
    log "Linking $HERMES_HOME_DIR/.env -> $PROJECT_ROOT/.env"
    if [ -f "$HERMES_HOME_DIR/.env" ] && [ ! -L "$HERMES_HOME_DIR/.env" ]; then
        mv "$HERMES_HOME_DIR/.env" "$HERMES_HOME_DIR/.env.pre-symlink.bak"
        log "Existing ~/.hermes/.env backed up to .env.pre-symlink.bak"
    fi
